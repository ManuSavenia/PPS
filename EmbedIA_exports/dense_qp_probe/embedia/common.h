...
typedef struct {
    ...
    quant8 * data;       /**< Flattened data array in CHW order */
    qparam_t qparam;
} data3d_t;
...
typedef struct {
    ...
    quant8 * data;       /**< Flattened data array */
    qparam_t qparam;
} data2d_t;
...
typedef struct {
    ...
    quant8 * data;       /**< Contiguous array of values */
    qparam_t qparam;
} data1d_t;
...
/* ### FULL_QUANT8: int8/int32 dot product helper, no fixed-point dependency. ### */
EMBEDIA_INLINE int32_t dot_product(const quant8 *a, const quant8 *b, uint32_t len) {
    int32_t sum = 0;
